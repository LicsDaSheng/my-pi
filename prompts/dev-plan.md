...
本文件自包含：从任何目标仓库调用都不需要读取目标 cwd 下的 my-pi 文件，也不依赖固定安装路径。下文示例由主会话根据真实状态构造工具参数，不是需要执行的外部脚本。
...
- 将已确认的目标绝对路径记为 `const targetCwd = "<目标仓库绝对路径>"`（替换占位值）；所有配置预检 list/models/必要 get 和后续 workflow 执行必须显式使用同一 targetCwd，不依赖主会话 cwd。
...
- 不安装依赖、不修改目标项目文件、agent 或配置、不运行会改项目的测试／命令；只允许主会话保存本流程受管状态与产物。builtin scout 保持原定义，任务明确禁止写项目文件，报告只到运行时受管 output；不复制 scout/reviewer。

## 1. 状态和交接包

为本次流程生成唯一 workflow ID。分阶段调用 `workflowScript`，外层均 `async: true`、`cwd: targetCwd`（与配置预检相同的目标绝对路径）；不使用 worktree/mutation lane。等待原生完成通知后读取结果再推进，不在盲跑脚本中问用户，也不假定沙箱可调用主会话工具或访问文件系统。

首次 workflow 使用运行时默认 mission，记录返回的真实 missionId；后续显式传相同 missionId。用 `await state.set("dev-plan", <JSON快照>)` 保存，恢复用 `await state.get("dev-plan")`。每次启动前及完成、用户回答、裁决、暂停、交付后更新。不要 `mission:false`。脚本没有 fs；主会话将原文／答案作为 JSON 数据安全序列化传入，禁止拼接用户文本为可执行代码。状态过大时将长文存入受管 output，状态保存真实引用（mission state 上限 256 KiB），保存失败立即暂停。

快照必须包含：
- workflow ID、missionId、原始需求（或完整原文引用）与摘要；
- 仓库／分支／提交／工作区状态及采集时间；
...
- 各阶段唯一 key、外层运行 ID、子任务最新 runId、实际模型、实际 output 引用；
- 当前状态、下一步、基础设施错误、是否已获用户方案确认。

每次子任务交接必须完整提供：原需求、目标／基线、边界、代码证据、所有相关原始回答／决策／假设、当前方案及版本、模式、验收要求和真实产物引用。fresh 会话须能独立理解；引用不存在或被清理先暂停补证据，不能假装读过。

阶段 key 示例：`<workflow-id>.scout.1`、`.clarify.1`、`.draft.v1`、`.review.v1.r1`、`.revise.v2`。每次新任务／恢复使用新 key，不覆盖旧产物。

## 2. 探索 → 澄清暂停点

用 `runs.run(key, { agent: "scout", context: "fresh", task: <完整任务>, output: <唯一相对产物名> })`。外层 async 为 true；所有后续阶段同样后台调用，不将 async 误作 runs.run 的控制开关；每次调用均不传 per-run model 参数。任务约束：
...
成功且输出引用可用后，启动 fresh development-designer，依赖用户级 settings 中的角色覆盖路由，模式 `clarify`。任务要求输出 `ready_to_draft` 或 `needs_user_input` 及理由。关键未知项未解决不得进入可开发结论。
...
- 正常问卷由 designer 最终响应返回，本轮结束；用户回答后再恢复。若子任务通过 `contact_supervisor` 发起运行中阻塞访谈，主会话调用问卷后走该桥接回复，不再并发 resume 同一子任务。

## 3. 保留设计会话与起草

每次继续访谈或修订前调用 `subagent({ action: "children.list" })`。只有最新 designer runId 明确 `resumable` 才使用新 key 的 `runs.run(key, { resume: <最新runId>, task: <完整回传>, output: <新产物名> })`。resume 不传 agent/context 覆盖存储契约，核对其仍为指定设计角色且有效模型来自用户级 settings。返回的新 runId 替换旧 ID。

不可恢复、条目缺失、跨主会话无法恢复时，新开同角色 fresh 会话，仍依赖同一用户级 settings 角色覆盖并提供完整交接，记录恢复失败原因；这不是换模型。真正 resume/基础设施执行错误先暂停并报告精确错误及状态，确认无在途旧任务后，只能经明确的同协议恢复再启动同角色，不能静默重试或换 CLI。

澄清完成后以 `draft` 模式起草 v1。要求稳定 REQ/AC ID（不随版本重排／复用），完整方案而非要点，优先复用最小设计；明确范围内外、约束、接口／数据／失败处理／兼容性、依赖步骤及验证方法。计划测试不得写成已执行。
...
每次评审均 fresh reviewer，依赖用户级 settings 中的角色覆盖路由；交接原需求、决策、探索证据、完整方案与精确版本；复核额外提供上一轮问题和逐条处置记录。不继承设计会话历史。

在启动任何 reviewer 前先检查 `review_count < review_limit`，然后增加并保存 review_count；发起过的失败／格式错误评审也占一次，不因重试重置。超过上限不得再启动，见第 5 节。

任务明确：评审的是方案而非实现；检查需求覆盖／范围、实际代码与契约、模块／接口／数据／错误／兼容性、最小方案、步骤依赖、可观察验收、异常边界安全回归、未确认假设。禁止修改方案、替用户决策、用个人偏好制造阻塞；没有实现代码不是缺陷。本任务输出格式替代默认 Merge verdict。

每次 `runs.run` 的 `outputSchema` 必须使用下面完整 JSON Schema（不使用自然语言“通过”搜索）：
...
产物规则：
- 每个子任务显式 `output`，采用 `<workflow-id>/<stage-version>/...` 唯一相对名让运行时路由到受管目录，不指定目标项目绝对文件路径。完整方案的 basename 为 `development-plan.md`，各版本分目录保存。
- 只读 designer/reviewer 返回完整内容，由运行时保存；不要求它们调用 write。不要把任务文本里的期望路径当成真实引用。
- 脚本返回子结果的 `outputReference`、`outputPathMapping` 或 `artifactPaths`（按当前返回结构取真实引用）、runId、structuredOutput；async 外层 runId 不等于设计子会话 ID。状态及最终消息使用实际返回引用，检查文件可读且内容完整后交付。
